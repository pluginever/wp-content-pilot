...
class WPCP_HTML {
	 ...
	 * Text input.
	 *
	 * @param array $field Campaign option fields.
	 *
	 * @since 1.0.0
	 * @return string
	 */
	public static function text_input( $field ) {
		...
		$meta_value = get_post_meta( $thepostid, $field['name'], true );
		if ( empty( $args['value'] ) && $meta_value ) {
			$args['value'] = (string) $meta_value;
		} else {
			$args['value'] = $args['default'];
		}

		$attributes = self::implode_html_attributes(
			$args['attrs'],
			array(
	...
	public static function textarea_input( $field ) {
		...
		$args['id'] = esc_attr( ! empty( $args['id'] ) ? $args['id'] : $args['name'] );
		$meta_value = get_post_meta( $thepostid, $args['name'], true );
		if ( empty( $args['value'] ) && ! empty( $thepostid ) && $meta_value ) {
			$args['value'] = (string) $meta_value;
		} else {
			$args['value'] = $args['default'];
		}
	...
	public static function select_input( $field ) {
		...
		$args['id'] = esc_attr( ! empty( $args['id'] ) ? $args['id'] : $args['name'] );
		$meta_value = get_post_meta( $thepostid, sanitize_key( $field['name'] ), true );
		if ( empty( $args['value'] ) && ! empty( $thepostid ) && $meta_value ) {
	...
	public static function input_desc( $field ) {
		...
		return sprintf(
			'<span class="wpcp-input-desc">%s</span>',
			wp_kses(
				$field['desc'],
				array(
					'a'      => array(
	 ...
	 *
	 * @param array $raw_attributes Raw attributes.
	 * @param array $extra Array of attributes.
	 *
	 * @since 1.0.0
	 * @return string
	 */
	...
	/**
	 * Start the double column.
	 *
	 * @param string $class_name HTML class.
	 *
	 * @since 1.2.0
	 * @return string
	 */
	public static function start_double_columns( $class_name = '' ) {
		return sprintf( '<div class="wpcp-dc %s">', sanitize_html_class( $class_name ) );
	}

	/**
	 * End the double column.
	 *
	 * @since 1.2.0
	 * @return string
	 */
