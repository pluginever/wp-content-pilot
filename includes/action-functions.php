...
function wpcp_handle_manual_campaign() {
	...
	$message       = sprintf( __( 'A post successfully created by %1$s titled %2$s', 'wp-content-pilot' ), '<strong>' . get_the_title( $campaign_id ) . '</strong>', $article_title );
...
function wpcp_ajax_run_manual_campaign() {
	...
	global $current_user;
	wpcp_logger()->info( sprintf( __( 'Campaign <strong>%1$s</strong> manually initiated by <strong>%2$s</strong>', 'wp-content-pilot' ), get_the_title( $campaign_id ), $current_user->display_name ) );
	...
	$message       = sprintf( __( 'A post successfully created by campaign %1$s titled %2$s', 'wp-content-pilot' ), '<strong>' . get_the_title( $campaign_id ) . '</strong>', $article_title );
	wp_send_json(
		[
			'message' => $message,
			'link'    => $article_title,
