...
class Elements {
	...
	public function file( $args ) {
		if ( empty( $args['class'] ) ) {
			$args['class'] = 'ever-file-upload';
		} elseif ( ! strpos( $args['class'], 'ever-file-upload' ) ) {
			$args['class'] .= ' ever-file-upload';
		}
	...
	public function avatar_upload( $args = array() ) {
		$args = wp_parse_args( $args, array(
			'name'        => '',
			'placeholder' => EVER_HRM_ASSETS_URL . '/images/man.png',
			'input_name'  => '',
			'value'       => '',
		) );
		ob_start();
		?>
		<div class="ever-avatar-upload">
			<div class="ever-avatar-edit">
				<input type='file' id="ever-avatar-upload" accept=".png, .jpg, .jpeg" data-nonce="<?php echo wp_create_nonce( 'ehrm_upload_image' ); ?>"/>
				<label for="ever-avatar-upload"></label>
			</div>
			<div class="ever-avatar-preview-warp">
				<div class="ever-avatar-preview" style="background-image: url(<?php echo empty( $args['value'] ) ? esc_attr( $args['placeholder'] ) : $args['value']; ?>);"></div>
			</div>
			<input type="hidden" class="ever-avatar-value" name="<?php echo esc_attr( $args['input_name'] ); ?>" value="<?php esc_url( $args['value'] ); ?>">
		</div>
		<?php
